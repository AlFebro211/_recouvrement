...
class CategorieOperation(models.Model):
    ...
    type_operation = models.CharField(max_length=10,choices=[('ENTREE', 'Entrée d\'argent'),('SORTIE', 'Sortie d\'argent')],)
    ...
    def __str__(self):
        return f"{self.type_operation}-{self.nom} ({self.id_annee.annee} - {self.id_campus.campus})"

class OperationCaisse(models.Model):
    id_operation = models.AutoField(primary_key=True)
    id_annee = models.ForeignKey('Annee', on_delete=models.PROTECT)
    id_campus = models.ForeignKey('Campus', on_delete=models.PROTECT)
    categorie = models.ForeignKey(CategorieOperation,on_delete=models.PROTECT,related_name='operations')
    montant = models.IntegerField(verbose_name="Montant")
    date_operation = models.DateField(verbose_name="Date")
    date_enregistrement = models.DateTimeField(auto_now_add=True)
    description = models.CharField(max_length=255,blank=True,null=True)
    source_beneficiaire = models.CharField(max_length=200,blank=True,null=True,help_text="Source (entrée) ou bénéficiaire (sortie)")
    mode_paiement = models.CharField(max_length=20,choices=[('ESPECES', 'Espèces'),('CHEQUE', 'Chèque'),('VIREMENT', 'Virement'),('AUTRE', 'Autre')],default='ESPECES')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['id_annee', 'id_campus', 'date_operation']),
        ]
